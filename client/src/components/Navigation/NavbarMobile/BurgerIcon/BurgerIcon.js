    ...
    <ul title="Menu" onClick={() => setIsOpen(!isOpen)}>
      <li>
        <Transition
          // appear={true}
          in={isOpen}
          onEnter={node => burgerIconAnimation(node, topRef.current, bottomRef.current, 0.5, true)}
          onExit={node => burgerIconAnimation(node, topRef.current, bottomRef.current, 0.35, false)}
